...
public class AboutWebIntegrationTest extends AbstractWebIntegrationTest {
    ...
    public void shouldRedirectForMissingTrailingSlash() throws Exception {
        try {
            ...
            client.resource(BASE).path("/about").get(String.class);
            fail("should throw");
        } catch (UniformInterfaceException e) {
            assertThat(e.getResponse().getStatus(), equalTo(HttpServletResponse.SC_FOUND));
            assertThat(e.getResponse().getLocation(), equalTo(BASE.resolve("/about/")));
        }
    }
    
    @Test
    public void shouldHaveCorrectTitle() throws Exception {
        String response = Client.create().resource(BASE).path("/about/")
