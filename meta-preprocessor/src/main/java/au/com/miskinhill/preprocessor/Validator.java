...
import au.com.miskinhill.rdf.vocabulary.BIBO;
...
public class Validator {
    ...
    private static final Set<Property> DOMAIN_PROPERTY_EXCEPTIONS = Collections.unmodifiableSet(new HashSet<Property>(Arrays.asList(
            BIBO.editor)));
    ...
    private static final Set<Property> RANGE_PROPERTY_EXCEPTIONS = Collections.unmodifiableSet(new HashSet<Property>(Arrays.asList(
            ...
            /* XXX can fix these two */ BIBO.contributorList, BIBO.authorList)));
    ...
    public void validate() throws Failure {
        ...
        for (StmtIterator it = m.listStatements(null, RDFS.domain, (RDFNode) null); it.hasNext(); ) {
            ...
            if (DOMAIN_OBJECT_EXCEPTIONS.contains(domainConstraint.getObject()) ||
                    DOMAIN_PROPERTY_EXCEPTIONS.contains(domainConstraint.getSubject().as(Property.class)))
                continue;
